...
class StdDevUnder(ConditionFromMeter):
    r"""Returns True when the standard deviation of a given meter
    over the latest values is lower than some threshold.

    Each time this is called, it will read the value of the ``meter`` and
    will store it.
    ...
    def __init__(self, meter, threshold, num_values_to_keep,
        ...
        super(StdDevUnder, self).__init__(meter, meter_key, name)
