...
class CTCAlignmentTest(unittest.TestCase):
    def setUp(self):
        self._logp_matrix = np.log(np.asarray([
            [0.3, 0.5, 0.2],
            [0.4, 0.5, 0.1],
            [0.5, 0.1, 0.4],
            [0.1, 0.7, 0.2]]))
    ...
    def empty_reference_test(self):
        best_logp, best_alig = ctc_alignment(self._logp_matrix,
                                             [],
                                             ctc_sym=0)
        np.testing.assert_almost_equal(best_logp,
                                       math.log(0.3 * 0.4 * 0.5 * 0.1))
