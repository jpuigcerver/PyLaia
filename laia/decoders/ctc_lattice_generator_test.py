...
try:
    ...
    class CTCLatticeGeneratorTest(unittest.TestCase):
        def setUp(self):
            self._x = torch.Tensor([[[1, 2, 3], [0, 0, 1]],
                                    [[-1, 0, 2], [0, 3, 0]],
                                    [[2, 0, 0], [3, 1, 2]],
                                    [[-1, -2, -3], [-3, -2, -1]]])
            self._packed_x = pack_padded_sequence(
                input=Variable(self._x, requires_grad=False),
                lengths=[3, 2])
        ...
        def tensor_normalize_test(self):
            ...
            lats = latgen(self._x)
            self.assertTrue(
                fst.equivalent(lats[0], self.normalize_fst(self._fst1())))
            self.assertTrue(
                fst.equivalent(lats[1], self.normalize_fst(self._fst2())))

        def packed_tensor_normalize_test(self):
            latgen = CTCLatticeGenerator(normalize=True)
            lats = latgen(self._packed_x)
            self.assertTrue(
                fst.equivalent(lats[0],
                               self.normalize_fst(self._fst1_packed())))
            self.assertTrue(
                fst.equivalent(lats[1],
                               self.normalize_fst(self._fst2_packed())))
