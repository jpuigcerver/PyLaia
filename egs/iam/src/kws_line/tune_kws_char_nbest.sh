...
latdir=data/kws_line/lats/resize_h128/char_8gram;
...
mkdir -p "$1" || exit 1;
for n in 10 18 30 100 300 900 2700 8100 24300; do
  logfile="$1/tune_acoustic_prior_scale_segment_char_8gram_${n}best.log";
  [ -s "$logfile" ] ||
  ...
  readarray -t size_time < \
      <(for p in va te; do
          (
            time lattice-char-index-segment \
            --nbest=$n \
            "${delimiters[*]}" \
            "ark:$latdir/ps${params[1]}/$p.lat.ark" ark,t:- 2>/dev/null
          ) 2>&1 |
          awk '
          BEGIN{
            IDX = 0;
          }{
            if (NF == 2 && $1 == "real") TIME=$2;
            else if (NF == 2 && ($1 == "user" || $1 == "sys")) next;
            for (i = 2; i <= NF; i+=5) {
              IDX++;
            }
          }END{ print IDX; print TIME; }'
        done) || exit 1;
  echo ${size_time[@]};
