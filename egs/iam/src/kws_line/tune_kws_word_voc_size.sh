...
for vs in 10k 20k 30k 40k 50k 60k 70k 80k 90k 100k; do
  latdir="data/kws_line/lats/resize_h128/word_3gram_${vs}";
  check_all_dirs "$latdir" || exit 1;
  check_all_files -s "data/kws_line/lm/word_3gram_${vs}/words.txt" || exit 1;

  logfile="$1/tune_acoustic_prior_scale_segment_resize_h128_word_3gram_${vs}.log";
  [ -s "$logfile" ] ||
  ( ./src/kws_line/tune_metric_kws_word.py \
    --queries=data/kws_line/lang/queries/iam_old_papers_queries.txt \
    --index-type=segment \
    "data/kws_line/lm/word_3gram_${vs}/words.txt" \
    data/kws_line/lang/kws_refs/va.txt \
  ...
  gawk -v vs="$vs" -v as="${params[0]}" -v ps="${params[1]}" \
       -v va_map="${va_map_gap[0]}" -v va_gap="${va_map_gap[1]}" \
  '{
    if (match($0, "'\''mAP'\'': ([0-9.]+)", mAP) &&
        match($0, "'\''gAP'\'': ([0-9.]+)", gAP)) {
      printf("%3-s %.9f %.9f %.9f %.9f (as = %.2f  ps = %.1f)\n",
             vs, va_map, va_gap, mAP[1], gAP[1], as, ps);
    } else {
      print "Wrong last line: "$0 > "/dev/stderr"; exit(1);
    }
  }'
