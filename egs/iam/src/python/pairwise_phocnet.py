...
if __name__ == "__main__":
    ...
    if args.distractors:
        ...
        log.info("Computing distances between {} queries and {} distractors", Q, D)
        ...
        distract_inds = [(distract_inds[k], k) for k in np.argsort(distract_distances)]
    else:
        distract_distances = []
        distract_inds = []
    ...
    def print_query_dist(i, j, k):
        args.output.write(
            "{} {} {}\n".format(query_ids[i], query_ids[j], query_distances[k])
        )
        args.output.write(
            "{} {} {}\n".format(query_ids[j], query_ids[i], query_distances[k])
        )

    def print_distract_dist(i, j, k):
        args.output.write(
            "{} {} {}\n".format(query_ids[i], distract_ids[j], distract_distances[k])
        )

    qk, dk = 0, 0
    while qk < len(query_inds) and dk < len(distract_inds):
        if (
            query_distances[query_inds[qk][1]]
