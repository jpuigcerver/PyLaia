...
BEGIN{
  while ((getline < f) > 0) {
    ...
    if (pair in SA) {
      ...
      else if (c) { SA[pair] = log(w) - $3; }
      else if (l) { SA[pair] = log(w) + $3; }
      else { SA[pair] = log(w) + log($3); }
    }
  }
}{
  pair = $1" "$2;
  if ($3 == "inf" || $3 == "-inf") { sB = "-inf"; }
  else if (c) { sB = log(1.0 - w) - $3; }
  else if (l) { sB = log(1.0 - w) + $3; }
  else { sB = log(1.0 - w) + log($3); }

  if (pair in SA) {
    if (pair in DONE) {
      print "WARNING: Pair \""pair"\" was repeated in index2" > "/dev/stderr";
    } else {
      print pair, logadd(SA[pair], sB);
      DONE[pair] = 1;
    }
  } else {
