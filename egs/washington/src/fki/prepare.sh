...
sed -r 's|<space> (<space> )*|<space> |g' > data/fki/lang/char/transcript.txt;
...
for cv in cv1 cv2 cv3 cv4; do
  mkdir -p "data/fki/lang/char/$cv";
  for p in test train valid; do
    [[ "$overwrite" = false && -s "data/fki/lang/char/$cv/${p:0:2}.txt" ]] ||
    join -1 1 \
	 <(sort data/fki/lang/char/transcript.txt) \
	 <(sort "data/fki/washingtondb-v1.0/sets/$cv/$p.txt") |
    sort -V > "data/fki/lang/char/$cv/${p:0:2}.txt" || exit 1;
  done;
done;
